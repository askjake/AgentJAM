...
from .message_summarizer import get_summarizer
...
async def call_model(state: AgentState, config=None):
    ...
    # Apply summarization strategy instead of hard truncation
    if len(messages) > 100:
        summarizer = get_summarizer()
        messages = summarizer.summarize_messages(messages)
    ...
    # Call the model
    # Apply methodology injection if triggered by user message
    # Extract the last user message to check for triggers
    user_message = ""
