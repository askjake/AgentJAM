...
def is_log_file(filename: str, content_type: str) -> bool:
    ...
    Log files are stored in agent sandbox rather than being embedded.
    Supports numbered extensions like .001, .002, .log.1, etc.
    """
    ...
    # Check for numbered extensions (common for logs and split archives)
    # Pattern 1: .log.1, .log.2, etc.
    if re.match(r'.*\.log\.\d+$', filename_lower):
        return True
    
    # Pattern 2: Numbered extensions like .001, .002, .003 (split archives)
    if re.match(r'.*\.\d{3,}$', filename_lower):
        return True
    
    # Pattern 3: Archive with numbered extension like .zip.001, .tar.gz.001
    if re.match(r'.*\.(zip|tar|gz|tgz|rar|7z)\.\d+$', filename_lower):
        return True
    
    # Check standard log/archive extensions
